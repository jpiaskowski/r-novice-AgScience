...
Data types are the building blocks of any analytic software. Most logically, they define the broad categories of how R views data and how it is stored on your system. We can ask what type of data something is with `typeof()`:
...
You can check if something is a vector and its type:
...
The coercion rules go: `logical` -> `integer` -> `numeric` -> `complex` -> `character`, where -> can be read as *are transformed into*. You can try to force coercion against this flow using the `as.xxxxx()` functions. This includes `as.logical()`, `as.character()`, and `as.numeric()`. 
...
We can look at individual elements in a vector by indexing their position with the square brackets `[ ]`. It may look different, but the square brackets operator is a function. For vectors, it means "get the nth element".

Indexing starts at 1 for the first element and ends at the length of the vector. Length can be assessed with the function `length()`.
...
Now R has noticed that there are three possible categories in our data - but it also did something surprising; instead of printing out the strings we gave it, we got a bunch of integers instead. R has replaced our human-readable categories with numbered indices under the hood, this is necessary as many statistical calculations utilise such numerical representations for categorical data:

```{r}
str(Trt)
```

Factors are tricky-- it's beneficial to know how to manipulate them. The first thing that is helpful to know if how to order factors. In many statistical models, the first level of a factor as set as the baseline and the other levels are compared to that point. In R, the baseline level is first level, but by default, factors are labelled in alphabetical order. You can change this by specifying the levels:
...
> **Tip:** There is a fantastic program for manipulating factors on CRAN called [**forcats**](https://forcats.tidyverse.org/). 
...
> ## Tip: Vector numbering in R starts at 1
>
